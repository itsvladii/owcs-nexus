//MANUAL SYNC SCRIPT FOR ELO RANKINGS FOR FORCED SYNCING AS A LAST RESORT
...
async function runSync() {
  const apiKey =
    import.meta.env?.LIQUIPEDIA_API_KEY || process.env.LIQUIPEDIA_API_KEY;
  ...
  let initialTeams:
    | Record<string, import("../../lib/elo/calcELO").RatedTeam>
    | undefined;
  ...
  await supabase
    .from("rankings")
    .upsert(cleanRankings, { onConflict: "name" })
    .limit(1);
  await supabase
    .from("global_stats")
    .upsert(cleanStats, { onConflict: "id" })
    .limit(1);
