...
function getTraderTags(assets: PortfolioItem[], netWorth: number, transactions: TransactionData[]): TraderTag[] {
  ...
  if (buyCount > 0 && sellCount === 0) {
    tags.push({ label: 'Diamond Hands', icon: '💎', color: 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20', desc: 'Refuses to sell.' });
  } else if (totalCount > 10 && sellCount > buyCount) {
    tags.push({ label: 'Paper Hands', icon: '🥬', color: 'bg-lime-500/10 text-lime-400 border-lime-500/20', desc: 'Panic seller.' });
  } else if (totalCount >= 20) {
    tags.push({ label: 'Day Trader', icon: '⚡', color: 'bg-orange-500/10 text-orange-400 border-orange-500/20', desc: 'High volume trader.' });
  }
...
export default function PortfolioDashboard() {
  ...
  // Stats
  const [cashBalance, setCashBalance] = useState(0); // <--- NEW: Liquid Cash
  ...
  const fetchPortfolioData = async () => {
    try {
      ...
      if (portfolioRaw && teamsRaw && txRaw) {
        ...
        setTotalDividends(dividendTxs.reduce((sum, t) => sum + (t.price || 0), 0));
        setRecentPayouts(dividendTxs.slice(0, 5));

        const payerMap: Record<string, number> = {};
        dividendTxs.forEach(t => { payerMap[t.team_name] = (payerMap[t.team_name] || 0) + t.price; });
        const bestPayerName = Object.keys(payerMap).reduce((a, b) => payerMap[a] > payerMap[b] ? a : b, '');
        if (bestPayerName) setTopPayer({ name: bestPayerName, amount: payerMap[bestPayerName] });
      }
    } catch (error) {
  ...
  const calculateFinancials = (portfolio: any[], teams: TeamData[], transactions: TransactionData[], cash: number) => {
    ...
    setNetWorth(totalStockValue + cash); 
    
    setTraderTags(getTraderTags(calculatedAssets, totalStockValue + cash, transactions));
  };
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
     ...
     <div className="grid grid-cols-1 xl:grid-cols-12 gap-6">
        ...
        {/* LEFT COLUMN (8 cols): Your Charts & Tables */}
        <div className="xl:col-span-8 space-y-6">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-2xl h-[320px] flex flex-col">
              ...
              <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-2xl h-[320px] flex flex-col">
                  <h3 className="text-white font-bold mb-4 text-sm uppercase tracking-wide flex items-center justify-between">
                      ...
                      <span className="text-[10px] bg-emerald-500/10 text-emerald-400 px-2 py-0.5 rounded border border-emerald-500/20 animate-pulse">Live</span>
                  </h3>
                  <div className="space-y-3 overflow-y-auto custom-scrollbar flex-1 pr-2">
                      {recentPayouts.length === 0 ? (
                          ...
                          recentPayouts.map((tx, i) => (
                              <div key={i} className="flex justify-between items-center border-b border-neutral-800 pb-3 last:border-0 last:pb-0">
                                  <div className="flex items-center gap-3">
                                      <div className="w-8 h-8 rounded bg-neutral-800 flex items-center justify-center text-xs font-bold text-neutral-500 shrink-0">
                                        {tx.team_name.substring(0, 2).toUpperCase()}
                                      </div>
                                      <div className="min-w-0">
                                        <div className="text-white text-sm font-bold truncate">{tx.team_name}</div>
           ...
           <div className="bg-neutral-900 border border-neutral-800 p-6 rounded-2xl">
              ...
              {assets.length === 0 && <div className="text-center py-10 text-neutral-600 italic">Your portfolio is empty. Time to buy!</div>}
           </div>

        </div>

        {/* RIGHT COLUMN (4 cols): The F1 Leaderboard */}
        <div className="xl:col-span-4 h-[600px] xl:h-auto sticky top-6">
