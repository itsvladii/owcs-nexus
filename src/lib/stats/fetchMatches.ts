...
export async function fetchRecentMatches(
  ...
  try {
    ...
    endpoint.searchParams.set('conditions', `[[finished::1] AND ([[date::>2025-01-24]] AND [[date::<2025-02-04]]) AND ([[liquipediatier::1]] OR [[liquipediatier::2]])`);
...
export async function fetchAllSeasonMatches(apiKey: string, userAgent: string) {
  ...
  try {
    ...
    endpoint.searchParams.set('conditions', '[[finished::1]] AND [[date::>2026-02-13]] AND ([[liquipediatier::1]] OR [[liquipediatier::2]]) AND ([[series::Overwatch Champions Series]] OR [[series::Esports World Cup]])');
