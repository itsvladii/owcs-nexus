...
export function calculateRankings(
  ...
  // 2b. BIGGEST MOVER / LOSER — best & worst single match gain in the last 7 days
  // Using single-match ELO change avoids the issue where cumulative gains
  // inflate the stat on every sync after a match day.
  const recentMatches = processedMatches.filter(
    (m) => new Date(m.date) >= oneWeekAgo,
  );

  const matchMoves: Array<{
    name: string;
    diff: number;
    current: number;
  ...
  for (const m of recentMatches) {
    const tA = teams[m.team_a];
    const tB = teams[m.team_b];
    if (tA)
      matchMoves.push({
        name: m.team_a,
        diff: m.elo_change_a,
